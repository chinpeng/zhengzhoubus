...
<table id="tableList" class="tablesorter" cellspacing="1">
	...
	<tbody>
		<c:forEach var="sess" items="${voList}">
			<tr>
				...
				<td>
					...
					<c:when test="${sess.presence eq 'Online'}">
						<img src="<c:url value='/images/user-online.png'/>" />
					</c:when>
					<c:when test="${sess.presence eq 'Offline'}">
						<img src="<c:url value='/images/user-offline.png'/>" />
					</c:when>
					<c:otherwise>
						<img src="<c:url value='/images/user-away.png'/>" />
					</c:otherwise>
					</c:choose>
